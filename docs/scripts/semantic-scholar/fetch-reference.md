...
脚本会检查论文的 `referencesFetched` 状态：

- 如果为 `true`：跳过处理，提示已获取过
- 如果为 `false`：继续获取引用文献
...
处理完成后，将论文的 `referencesFetched` 状态更新为 `true`
...
- 论文的 `referencesFetched` 字段用于标记是否已获取过引用文献
- 避免重复获取同一篇论文的引用文献
- 如需重新获取，需要手动将 `referencesFetched` 设置为 `false`
...
A: 不会。脚本会检查 `referencesFetched` 字段，如果为 `true` 则跳过处理。

### Q: 如何重新获取某篇论文的引用文献？

A: 使用 Prisma Studio 或 SQL 将该论文的 `referencesFetched` 字段设置为 `false`，然后重新运行脚本。
